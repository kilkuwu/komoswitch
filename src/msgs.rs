...
impl UpdateWorkspaces {
    ...
    pub fn to_wmdmsg(workspaces: Vec<Workspace>) -> WndMsg {
    ...
    pub fn from_wndmsg(p: WndMsg) -> Vec<Workspace> {
        let workspaces = unsafe { Box::from_raw(p.lparam as *mut Vec<Workspace>) };
        *workspaces
    }
}
